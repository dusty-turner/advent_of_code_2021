...
g2 <- add_edges(make_empty_graph(n = width*width), as.matrix(t(my_graph[1:nrow(my_graph),1:2])), weight = my_graph[,3]$weight)
...
create_mat <- function(mat, iter){if_else(mat+1>9,1,mat+1) %>% 
...
g2 <- add_edges(make_empty_graph(n = width*5*width*5), as.matrix(t(my_graph[1:nrow(my_graph),1:2])), weight = my_graph[,3]$weight)
